...
const FETCH_RETRY_DELAYS_MS = [0, 5_000, 15_000, 30_000];
const RETRYABLE_HTTP_STATUS = new Set([408, 425, 429, 500, 502, 503, 504]);
export const SNAPSHOT_ALGORITHM_VERSION = "2026-07-24.1-public-source-redaction";
...
export function exportsBaseUrl() {
  ...
  if (!baseUrl) {
    throw new Error("MOONCAKES_EXPORTS_BASE_URL is required");
  }
  return baseUrl;
}
...
function buildWeeklyRegistrationHistory(normalizedUsers, ownerHistory, snapshotDate) {
  const ownerKeys = new Set(
    Object.keys(ownerHistory).map((owner) => String(owner).trim().toLowerCase())
  );
  const countsByWeek = new Map();

  for (const user of normalizedUsers) {
    if (user.registeredOn > snapshotDate) continue;
    const weekStart = startOfUtcCalendarWeek(user.registeredOn);
    if (!countsByWeek.has(weekStart)) {
      countsByWeek.set(weekStart, {
        registered_count: 0,
        enabled_registered_count: 0,
        disabled_registered_count: 0,
        contributed_count: 0,
        no_contribution_count: 0
      });
    }
    const counts = countsByWeek.get(weekStart);
    const isDisabled = parseBoolean(user.row.disabled);
    const hasContribution = ownerKeys.has(user.username.toLowerCase());
    counts.registered_count += 1;
    if (isDisabled) {
      counts.disabled_registered_count += 1;
      continue;
    }
    counts.enabled_registered_count += 1;
    if (hasContribution) counts.contributed_count += 1;
    else counts.no_contribution_count += 1;
  }

  const firstWeek = Array.from(countsByWeek.keys()).sort()[0] || "";
  const currentWeek = startOfUtcCalendarWeek(snapshotDate);
  const weeks = [];
  let cursor = firstWeek;
  let safety = 0;
  while (cursor && cursor <= currentWeek) {
    const counts = countsByWeek.get(cursor) || {
      registered_count: 0,
      enabled_registered_count: 0,
      disabled_registered_count: 0,
      contributed_count: 0,
    ...
    weeks.push({
      from: cursor,
      to: utcDateOffset(cursor, 6),
      ...counts
    });
    cursor = utcDateOffset(cursor, 7);
    safety += 1;
    if (safety > 600) throw new Error("Registration history spans too many weeks");
  }

  return {
    source_timezone: SOURCE_SIGNUP_TIMEZONE,
    display_timezone: "UTC",
    cohort_status: "contributed_by_snapshot",
    weeks
  };
}
...
function buildRegistrationAnalysis(userRows, ownerHistory, snapshotDate) {
  ...
  const history = buildWeeklyRegistrationHistory(normalizedUsers, ownerHistory, snapshotDate);
  const ownerKeys = new Set(
    Object.keys(ownerHistory).map((owner) => String(owner).trim().toLowerCase())
  );
  ...
  const withoutContribution = enabledLastWeek
    .filter(({ username }) => !ownerKeys.has(username.toLowerCase()))
...
export async function buildSnapshotFromExports(options = {}) {
  ...
  return {
    ...
    timezone: "UTC",
    source: {
      provider: "business-analytics-exports",
      datasets: [
        "users.csv",
        "packages.csv",
        "module-download-totals.csv"
      ]
    },
    modules,
    ...
    weekly_registration_history: registrationAnalysis.history,
